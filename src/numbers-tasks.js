...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const vectorMod = Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(scalarProduct / vectorMod);
}
...
function getFibonacciNumber(index) {
  const goldenSection = (1 + Math.sqrt(5)) / 2;
  return Math.round(goldenSection ** index / Math.sqrt(5));
}
